...
export const startup = (): void => {
  tw.wiki.addEventListener("change", changes => {
    if (!tw.utils.hop(changes, PLUGIN_UPDATE_RAW_TITLE)) return;
    ...
    try {
      ...
      const updatePlugins = JSON.parse(raw) as Record<string, [string, string?]>;
      ...
      const pluginsToShow = plugins.filter(title => {
        ...
        if (!lv) return false;
        if (lv[1] && tw.utils.compareVersions(tw.version, lv[1].trim()) < 0) return false;
        const v = tw.wiki.getTiddler(title)?.fields.version;
        return !(typeof v === "string" && lv[0] && tw.utils.compareVersions(v.trim(), lv[0].trim()) >= 0);
      });
      tw.wiki.addTiddler({
      ...
      if (pluginsToShow.length > 0) {
        tw.wiki.addTiddler({ title: "$:/temp/CPL-Repo/update-notification", text: "yes", count: String(pluginsToShow.length) });
      }
