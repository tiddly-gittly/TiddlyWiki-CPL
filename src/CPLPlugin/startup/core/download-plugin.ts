...
export const startup = (): void => {
  ...
  tw.wiki.addEventListener('change', changes => {
    if (!tw.utils.hop(changes, DOWNLOAD_PLUGIN_REQUEST_TITLE)) {
      return;
    }
    const tiddler = tw.wiki.getTiddler(DOWNLOAD_PLUGIN_REQUEST_TITLE);
    if (
      !tiddler ||
      typeof tiddler.fields.text !== 'string' ||
      !tiddler.fields.text
    ) {
      return;
    }
    ...
    const pluginTitle =
      typeof tiddler.fields.plugin === 'string'
        ? tiddler.fields.plugin
        : typeof tiddler.fields.title === 'string'
    ...
    const version =
      typeof tiddler.fields.version === 'string'
        ? tiddler.fields.version
        : 'latest';

    const url = `${getCurrentRepoEntry()}/plugins/${formatPluginTitle(
      pluginTitle,
    )}/${version}.json`;
