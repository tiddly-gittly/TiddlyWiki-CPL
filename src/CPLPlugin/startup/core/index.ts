...
const handleSearchPlugins = (text: string, saveTo: string): void => {
  if (searchLock || allPluginsCache === undefined || !pluginIndexCache) return;
  searchLock = true;
  try {
    if (text.length < 3) {
      tw.wiki.addTiddler({ title: saveTo, text: JSON.stringify(allPluginsCache), type: "application/json" });
      return;
    }
    tw.wiki.addTiddler({ title: "$:/temp/CPL-Repo/searching-plugin", text: "yes" });
    const patterns = new Set(text.split(/\s+/).map(p => p.toLowerCase()).slice(0, 10));
    const results = pluginIndexCache
      .map(plugin => {
        let weight = 0;
        for (const field of ["title", "author", "name"] as const) {
          const val = plugin[field];
          if (typeof val === "string") for (const p of patterns) if (val.toLowerCase().includes(p)) weight += 10;
        }
        if (plugin.tags) for (const tag of tw.utils.parseStringArray(plugin.tags)) if (patterns.has(tag.toLowerCase())) weight += 5;
        if (typeof plugin.description === "string") for (const p of patterns) if (plugin.description.toLowerCase().includes(p)) weight += 2;
        if (typeof plugin.description === "string") {
          const d = plugin.description.toLowerCase();
          for (const p of patterns) if (d.includes(p)) weight += 1;
        }
      ...
      .sort((a, b) => b[1] - a[1])
      .map(([t]) => t);
    tw.wiki.addTiddler({ title: saveTo, text: JSON.stringify(results), type: "application/json" });
    tw.wiki.deleteTiddler("$:/temp/CPL-Repo/searching-plugin");
  } catch (error) {
    console.error(error);
    tw.wiki.addTiddler({ title: "$:/temp/CPL-Repo/searching-plugin", text: String(error) });
  } finally { searchLock = false; }
};
    ...
    if (tw.utils.hop(changes, PLUGIN_INDEX_RAW_TITLE)) {
      ...
      try {
        ...
        for (const plugin of data) {
          ...
          if (plugin.category && plugin.category !== "Unknown") { categories[plugin.category] ??= []; categories[plugin.category].push(plugin.title); }
          if (plugin.author) { authors[plugin.author] ??= []; authors[plugin.author].push(plugin.title); }
          if (!plugin.title.startsWith("$:/languages") && plugin.title.split("/").length === 4) {
            const derivedAuthor = plugin.title.split("/")[2];
            if (derivedAuthor !== plugin.author) { authors[derivedAuthor] ??= []; authors[derivedAuthor].push(plugin.title); }
          }
          if (plugin.tags) for (const tag of tw.utils.parseStringArray(plugin.tags)) allTags.add(tag);
        }
        ...
        console.error(error);
        tw.wiki.addTiddler({ title: "$:/temp/CPL-Repo/getting-plugins-index", text: String(error) });
      }
    }

    if (tw.utils.hop(changes, SEARCH_PLUGINS_REQUEST_TITLE)) {
      const fields = tw.wiki.getTiddler(SEARCH_PLUGINS_REQUEST_TITLE)?.fields;
      if (!fields || typeof fields.text !== "string" || !fields.text) return;
