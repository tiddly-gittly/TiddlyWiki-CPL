...
/** 获得TW版本号 */
const getVersion =
  "$(npx tiddlywiki . --version | grep -Eo '^[0-9]+.[0-9]+.[0-9]+.*$' | head -n 1)";
...
function buildOnlineHTML(distDir, htmlName, minify, excludeFilter) {
  ...
  if (typeof htmlName !== 'string' || htmlName.length === 0)
    htmlName = 'index.html';
  ...
  shellI(
    `cp tiddlers/TiddlyWikiIconWhite.png ${distDir}/TiddlyWikiIconWhite.png &> /dev/null`,
  );
  shellI(
    `cp tiddlers/TiddlyWikiIconBlack.png ${distDir}/TiddlyWikiIconBlack.png &> /dev/null`,
  );
  ...
  shellI(
    'cd tiddlers && rm *.meta *.tid *.multids *.tiddler *.recipe *.txt *.css *.html *.htm *.hta *.js *.json *.md *.bib &> /dev/null',
  );
  ...
  if (minify) {
    shellI(
      `npx uglifyjs ${distDir}/tiddlywikicore.js -c -m --v8 --webkit --ie --output '${distDir}/tiddlywikicore-'${getVersion}'.js' && rm ${distDir}/tiddlywikicore.js`,
    );
    shellI(
      `npx html-minifier-terser -c scripts/html-minifier-terser.config.json -o ${distDir}/${htmlName} ${distDir}/index-raw.html && rm ${distDir}/index-raw.html`,
    );
  } else {
    shellI(
      `mv ${distDir}/tiddlywikicore.js '${distDir}/tiddlywikicore-'${getVersion}'.js'`,
    );
